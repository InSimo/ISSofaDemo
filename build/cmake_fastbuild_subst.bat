...
if not exist %tools_dir%\awk.exe set tools_dir=%cmake_source_dir%\..\support\ISExternals\tools
...
call :ResolvePath cmake_source_parent_dir "."
popd
...
REM BEGIN SUBST
echo @echo off > %cmake_build_dir%\setdrives.bat
echo if exist R:\ subst /D R: >> %cmake_build_dir%\setdrives.bat
echo subst R: "%cmake_build_dir%" >> %cmake_build_dir%\setdrives.bat
echo if exist S:\ subst /D S: >> %cmake_build_dir%\setdrives.bat
echo subst S: "%cmake_source_parent_dir%" >> %cmake_build_dir%\setdrives.bat
echo subst >> %cmake_build_dir%\setdrives.bat
echo Calling %cmake_build_dir%\setdrives.bat to setup R: and S: virtual drives (REDO after reboot or when switching between builds)
call %cmake_build_dir%\setdrives.bat
set cmake_build_dir=R:\
set cmake_source_dir=S:\build
R:
REM END SUBST
